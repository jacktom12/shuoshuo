(function () {
  const NETEASE_SONG_INFO_API = (id) => `https://music.zhheo.com/?server=netease&type=song&id=${id}&format=json`;
 const NETEASE_LRC_API = (id) => `https://api.qijieya.cn/meting/?server=netease&type=lrc&id=${id}`;
  const NETEASE_PLAYLIST_API = (id) => `https://music.zhheo.com/?server=netease&type=playlist&id=${id}`;
